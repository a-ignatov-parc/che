module.exports = (grunt) ->
  ...
  grunt.registerTask "update_sourcemap", "After concatenating app.js update it sourcemaps.", () ->
    ...
    if fs.existsSync sourcemapConfig.dest
      ...
      # Заменяем новую версию генерируемых sourcemap
      jsFileString = jsFileString.replace /\/\*\n\/\/@\ssourceMappingURL=.+\n\*\/;?/g, "/*\n//\n*/"

      # Заменяем старую версию генерируемых sourcemap
      jsFileString = jsFileString.replace /\/\/@\ssourceMappingURL=.+/g, ""

      jsFileString += "/*\n//@ sourceMappingURL=" + sourceMapFileName + ".map\n*/"
      fs.writeFileSync concatConfig.dest, jsFileString

      console.log "Sourcemap has been updated"

  grunt.registerTask "generate_sourcemap", "Harvesting files from sourcemaps generated by r.js and build propper sourcemaps.", () ->
    fs = require "fs"
    config = grunt.config "requirejs"
    outputFilePath = config.compile.options.out
    modulesRegex = /define\(['"]{1,1}([^'"]+)/

    if fs.existsSync outputFilePath
      outputFileString = fs.readFileSync outputFilePath, "utf-8"
  ...
  grunt.registerTask "require2", ["coffee","requirejs","concat:app","generate_sourcemap"]
